...
void mp_machine_pwm_init_helper(mp_hal_pwm_obj_t *self,
...
void mp_machine_pwm_deinit(mp_hal_pwm_obj_t *self) {
}


mp_obj_t mp_machine_pwm_freq_get(mp_hal_pwm_obj_t *self) {
    //return MP_OBJ_NEW_SMALL_INT(ledc_get_freq(self->mode, self->timer));
	return MP_OBJ_NEW_SMALL_INT(0);
}

void mp_machine_pwm_freq_set(mp_hal_pwm_obj_t *self, mp_int_t freq)
{

}

uint32_t get_duty_u16(mp_hal_pwm_obj_t *self) {
...
uint32_t get_duty_u10(mp_hal_pwm_obj_t *self) {
    //return ledc_get_duty(self->mode, self->channel) << (HIGHEST_PWM_RES + UI_RES_SHIFT - timers[TIMER_IDX(self->mode, self->timer)].duty_resolution);
	return 0;
}

mp_obj_t mp_machine_pwm_duty_get(mp_hal_pwm_obj_t *self) {
    return MP_OBJ_NEW_SMALL_INT(get_duty_u10(self));
}

void mp_machine_pwm_duty_set(mp_hal_pwm_obj_t *self, mp_int_t duty) {
    //set_duty_u10(self, duty);
}

uint32_t get_duty_ns(mp_hal_pwm_obj_t *self) {
    //return duty_to_ns(self, get_duty_u16(self));
	return 0;
}

mp_obj_t mp_machine_pwm_duty_get_u16(mp_hal_pwm_obj_t *self) {
    return MP_OBJ_NEW_SMALL_INT(get_duty_u16(self));
}

void mp_machine_pwm_duty_set_u16(mp_hal_pwm_obj_t *self, mp_int_t duty_u16) {
    //set_duty_u16(self, duty_u16);
}

mp_obj_t mp_machine_pwm_duty_get_ns(mp_hal_pwm_obj_t *self) {
    return MP_OBJ_NEW_SMALL_INT(get_duty_ns(self));
}

void mp_machine_pwm_duty_set_ns(mp_hal_pwm_obj_t *self, mp_int_t duty_ns) {
